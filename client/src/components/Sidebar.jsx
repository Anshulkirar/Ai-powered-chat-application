    ...
    <div className={`flex flex-col h-screen min-w-72 p-5 dark:bg-gradient-to-b from-[#242124]/30 to-[#000000]/30 border-r border-[#80609F]/30 backdrop-blur-3xl transition-all duration-500 max-md:absolute left-0 z-10 ${!isMenuOpen && 'max-md:-translate-x-full'}`}>
      ...
      <div className="flex items-center gap-2 p-3 mt-4 border border-gray-400 dark:border-white/20 rounded-md">
        ...
        <input
          ...
          className="text-xs placeholder:text-gray-400 outline-none dark:text-white"
        />
      </div>

      {/* Recent Chats */}
      {chats.length > 0 && <p className="mt-4 text-sm dark:text-white">Recent Chats</p>}
      <div className="mt-3 text-sm space-y-3 overflow-y-auto" style={{ maxHeight: 'calc(100vh - 350px)' }}>
        {chats
          .filter((chat) =>
            chat.messages[0]
      ...
      >
        ...
        <p className="text-sm dark:text-white">Community Images</p>
      </div>
      ...
      >
        ...
        <div className="flex flex-col text-sm">
          ...
          <p className="text-xs text-gray-400 dark:text-[#B1A6C0]">Purchase credits to use quickgpt</p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md cursor-pointer group">
        ...
        <p className="flex-1 text-sm truncate dark:text-white">{user ? user.name : 'Login your account'}</p>
